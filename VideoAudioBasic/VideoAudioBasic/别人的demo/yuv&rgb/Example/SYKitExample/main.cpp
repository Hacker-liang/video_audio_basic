...
void testI420ToRgb24()
{
    ...
    unsigned char *yuv   = (unsigned char *)malloc(500*500*3/2);
    unsigned char *rgb24 = (unsigned char *)malloc(500*500*3);
    if (NULL == yuv || NULL == rgb24)
    {
        printf("Malloc data buffer failure!\n");
        return;
    }
    FILE *fyuv   = fopen("output_rgb2_yuv1.yuv", "rb+");   // 打开 YUV 文件
    ...
    for (int i = 0; i < LOOP_COUNT; i++)
    {
        ...
        {
            // 清空内存
            memset(yuv, 0, 500*500*3/2);
            memset(rgb24, 0, 500*500*3);
            
            fread(yuv, 1, 500*500*3/2, fyuv);  // 每次读取一帧 YUV 数据
            
            converter.SY_I420ToRgb24(yuv, 500, 500, rgb24);    // 转换成 RGB24
            
            fwrite(rgb24, 500*500*3, 1, frgb24);
        }
        endTime = currTime();
